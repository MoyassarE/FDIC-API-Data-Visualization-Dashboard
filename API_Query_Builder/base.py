# -*- coding: utf-8 -*-
"""
Created on Sun May  5 15:46:30 2024
...
from metadata.__init__ import meta_dict
...
class BF:
    ...
    def _construct_params(
        ...
        params = {
            ...
            'sort_order' : kwargs.get('sort_order', d['sort_order']),
            'limit' : kwargs.get('limit', d['limit']),
            'offset' : kwargs.get('offset', d['offset']),
            'format' : 'csv' if kwargs.get('output') == 'pandas' else 'json',
            'download' : 'false',
            # if user provides data to query, use it. Otherwise, query all data within dataset
            'fields' : kwargs.get('fields', ','.join(list(meta_dict[key].keys())))
        }
        # if a filter is provided, add new filter item to dictionary
        if filters:
            params.update({'filters' : filters})
        # if search is allowed and it is provided by the user, replace search parameter with user input
        if search and d['search']:
            params.update({'search' : search})
        return params
    
    def _friendly_fields(sef, key, data, dataframe=True):
        meta = meta_dict[key]
